...
import kaizenDownloadDataStore from './kaizenDownloadDataStore';

   const baseUrl = window.location.origin + window.location.pathname.replace(/\//g, '');
   const pathName = window.location.pathname;
   axios.defaults.baseURL = window.location.host == '172.70.10.166' ? '/ttkaizen/public' : ''
   ...
   const useStore = create((set, get) => ({
       ...
       loading: false,
       firstLoad: true,
       path: window.location.host == '172.70.10.166' ? '/ttkaizen/public' : '',
       pathFile: window.location.host == '172.70.10.166' ? '/ttkaizen/public/' : '',
       errorStatusText: "",
       toggleModal: (res, setRes) => {return setRes(!res)},
       toggleLoad: (res) => {set({ firstLoad: !res})},
       setUser: (user) => set({ user }),
       fetchDataAxios : async (res) => {
                        get().errorStatusText && set({errorStatusText : ""})
       ...
       logout: async() => {
                     ...
                     ...kaizenDownloadDataStore(set,get)
   }));
